...
export const authService = {
  login: async (credentials: { mobile: string; password: string }) => {
    ...
    const displayName =
      name && name.trim() !== "" ? name.trim() : credentials.mobile;
  ...
  getUserDashboard: async () =>
    apiClient("/auth/user/dashboard", { method: "GET" }),
  getAdminDashboard: async () =>
    apiClient("/auth/admin/dashboard", { method: "GET" }),
  ...
  isAdminLoggedIn: async (): Promise<boolean> =>
