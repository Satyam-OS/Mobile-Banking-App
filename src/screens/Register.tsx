...
export default function Register({ navigation }: any) {
  ...
  const captureImage = async () => {
    if (cameraRef.current) {
      ...
      setFormData({ ...formData, selfie: data.uri });
      setShowCamera(false);
    }
  };
  ...
  if (isPending) {
    return (
      <View
        style={
          [styles.mainContainer, { backgroundColor: "#F0F9FF" }] as ViewStyle[]
        }
      >
        <View style={styles.pendingContainer}>
          ...
          <Text style={styles.pendingDesc}>
            ...
            <Text style={{ fontWeight: "bold", color: "#001F3F" }}>
              NexusBank Premium
            </Text>{" "}
            is being processed. We will notify you once your KYC is verified.
          </Text>
        </View>
        <TouchableOpacity
              ...
              <View style={styles.stepView}>
                ...
                <View style={styles.row}>
                  <View style={{ flex: 1, marginRight: 10 }}>
                    ...
                    <TouchableOpacity
                      style={styles.pickerSubstitute}
                      onPress={() => setShowGenderModal(true)}
                    >
                      <Text
                        style={[
                          styles.pickerText,
                          !formData.gender && { color: "#94A3B8" },
                        ]}
                      >
                ...
                {Platform.OS !== "web" && showDatePicker && (
                  <DateTimePicker
                    value={
                      formData.dob ? new Date(formData.dob) : eighteenYearsAgo
                    }
                    mode="date"
            ...
            {step === 3 && (
              <View style={styles.stepView}>
                ...
                <UploadItem
                  ...
                  onPress={() => handleDocumentPick("aadhaarFront")}
                />
                <UploadItem
                  label="Aadhaar Card (Back) *"
                  file={formData.aadhaarBack}
                  onPress={() => handleDocumentPick("aadhaarBack")}
                />
                <UploadItem
                  label="PAN Card *"
                  file={formData.panCard}
                  onPress={() => handleDocumentPick("panCard")}
                />
              </View>
            )}
            ...
            {step === 4 && (
              <View style={styles.stepView}>
                ...
                <Text style={styles.infoText}>
                  Please provide a live selfie for face recognition. This is
                  required for secure account activation. *
                </Text>
              </View>
            )}
          </ScrollView>
...
const UploadItem = ({ label, file, onPress }: any) => (
  <TouchableOpacity
    style={[styles.uploadCard, file && styles.uploadCardDone]}
    onPress={onPress}
  >
    <View style={styles.uploadLeft}>
      <View style={styles.fileIcon}>
        <FileText size={18} color={file ? "#10B981" : "#0EA5E9"} />
      </View>
      <View>
...
const styles = StyleSheet.create({
  ...
  cameraControls: { marginBottom: 30 },
  captureBtn: {
    width: 70,
    height: 70,
    borderRadius: 35,
