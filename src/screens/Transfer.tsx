...
export default function Transfer({ navigation }: any) {
  ...
  // ❗ Only enforce max-limit rule here (NOT actual balance)
  ...
  // Error state for final insufficient balance check
  ...
  const handleNext = () => {
    if (step === "select" && selectedUser) {
      ...
      const numericPin = /^[0-9]{4}$/;
      if (!numericPin.test(password)) return;

      setStep("confirm");
    } else if (step === "confirm") {
      // ❗ Actual insufficient balance check happens here
              ...
              <View style={styles.amountInputArea}>
                <Text
                  style={[
                    ...
                    isAmountOverLimit && { color: "#EF4444" },
                  ]}
                >
                  ₹
                </Text>
              ...
              <View style={styles.noteBox}>
                ...
                <View style={styles.passwordInputWrapper}>
                  <TextInput
                    ...
                    value={password}
                    onChangeText={(txt) => {
                      if (/^[0-9]*$/.test(txt)) setPassword(txt);
                    }}
                  />
                  <TouchableOpacity
          ...
          {step === "success" && (
            <View style={styles.successContainer}>
              ...
              <Text style={styles.successDesc}>
                ₹{parseFloat(amount).toLocaleString("en-IN")} sent successfully
